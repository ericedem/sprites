...
angular.module('myApp.services', [], function($provide){
    ...
    $provide.factory('sheets',function(){
        return {
            /**
             * Input should be a string of nxm sizes
             * @param input
             */
            getSheets: function(input){
                var parts = input.split(' ');
                var images = [];
                var sheets = [];
                for (var i = 0; i < parts.length; i++){
                    var nm = parts[i].split('x');
                    var rect = new Rect(parseInt(nm[0],10),parseInt(nm[1],10));
                    images.push(rect);
                }
                ...
                while(images.length > 0){
                    ...
                    images = sheet.insertImages(images);
                    sheets.push(sheet);
                }
